        ...
        <div className="bg-white p-4 sm:p-6 rounded-3xl border border-slate-200 shadow-sm mb-10">
          ...
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:outline-hidden focus:border-emerald-500 focus:bg-white transition"
            />
          </div>
        ...
        {filteredResults.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredResults.map((item) => (
              <div
                key={item.id}
                className="p-6 rounded-3xl bg-white border border-slate-200 shadow-xs hover:border-emerald-400 hover:shadow-md transition-all duration-200 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <span className="px-2.5 py-1 rounded-md bg-emerald-50 text-emerald-800 text-[10px] font-black uppercase tracking-wider border border-emerald-200">
                      {item.category}
                    </span>
                  ...
                  <h4 className="text-sm font-extrabold text-emerald-700 mb-2">
                    {item.studentName}
                  </h4>
