...
const iconMap: Record<string, React.ReactNode> = {
  ...
  GraduationCap: <GraduationCap className="w-5 h-5 text-brand" />,
...
export default function StatsCounter() {
  return (
    <section className="bg-[#080e1e] border-y border-emerald-500/15 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
          {ECOSYSTEM_DATA.stats.map((stat, idx) => (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center mx-auto mb-3 text-brand group-hover:scale-110 transition-transform">
                {iconMap[stat.icon] || <Sparkles className="w-5 h-5 text-brand" />}
              </div>
              <div className="text-2xl font-black text-white tracking-tight group-hover:text-brand transition-colors">
                {stat.value}
              </div>
